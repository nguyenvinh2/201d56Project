...
var factPool = ['I like to watch epic Sax Man on repeat once a week', 'If I could be any fruit, I\'d be an apple. How about dem apples? ', 'My guilty pleasure is Ryan Gosling movies.', 'I am really from a small planet somewhere in the vincinity of betelgeuse', 'I write poetry about poetry.', 'I am the new Mayor of Quahog', 'I always have my towel.', 'I wear long sleeve shirts under short sleeve shirts under long sleeve shirts', 'Dont make me angry. You wouldn\'t like me when I\'m angry...', 'I want to know, WHAT\'S IN THE BOX??!!', 'There\'s a passage I learned that seemed appropriate for the moment: Ezekiel 25:17.', 'I don\'t want to be a product of my environment. I want my environment to be a product of me', 'I want to make a movie about a theme park with dinosaurs.. Original, right?', 'I like to sing Phil Collin\'s "In the Air Tonight", then subsequently get punched in the face by Mike Tyson.', 'I bowl really well with a hook hand.', 'Code Pairs? AALLLLLRIGHTY THEN!', 'I watch Tommy Boy just to hear Chris Farely say "holy schnikes".', 'I have a beautiful mind.... and I see people who aren\'t really there.', 'Me and my brother are Irish.. and vigilantes.', 'My perfect evening ends with Dwayne Johnson sing literally any song.', 'I think that "The Prestige" and "The Illuionist are the same"', 'I wouldn\'t want anyone meddling in my personal life. That\'s why I cleverly have no personal life.', 'If greatness was measured in water, I\'d be the Pacific', 'You\'re not superbad unless you watch "Superbad".', 'If I had a million dollars, I probably wouldn\'t be in this game right now.', 'I am NUNYA of the House BUSINESS...', 'In HBO\'s "Game of Thrones: Who Will Take the Iron Throne?" poll, why does Greyworm have 10% of the votes?', 'I wish I had a snickers.', 'I don\'t like turtles! :-p','On Thanksgiving Day, I watch every Rocky movie... In reverse order.','I\'ve seen better web games than this..'];
...
function cardDeckInit() {
  ...
  allCards[0].facts = '#Dr. Van Code: I have several furry companions. One of them I bring to work regularly; The awesome Demi of the House Hamm!!';
  ...
  allCards[3].facts = 'I am John\'s dog. I secretly rub my butt on John\'s pillow. You\'re welcome!';
...
function Score(score, userName) {
  ...
  this.toLocalStorage = function () {
    ...
    if (localStorage.getItem('game_1st') === null) {
      for (let i = 0; i < this.keys.length; i++) {
        this.scoresArr.push(['Unclaimed', 0]);
      }
    }
    ...
    for (let i = 0; i < this.scoresArr.length; i++) {
      ...
      if (this.score >= this.scoresArr[i][1] || this.scoresArr[i] === 'Unclaimed') {
  ...
  this.readHighScores = function () {
    let highScoreArr = [];
    for (let i = 0; i < this.keys.length; i++) {
      if (!(localStorage.getItem(this.keys[i]) === 'Unclaimed')) {
